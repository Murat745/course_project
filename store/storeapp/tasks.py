...
@shared_task
def books_avaibility():
    r = requests.get('http://warehouse:8001/books/')
    books_data = r.json()

    book_ids_in_data = [book_data['id'] for book_data in books_data]

    books_in_db = Book.objects.exclude(id_in_warehouse__in=book_ids_in_data)
    books_in_db.delete()

    for book_data in books_data:
        book, _ = Book.objects.update_or_create(
            id_in_warehouse=book_data['id'],
            defaults={
...
@shared_task
def orders_sync():
    r = requests.get('http://warehouse:8001/orders/')
    orders_data = r.json()

    for order_data in orders_data:
        order = Order.objects.get(id=order_data['order_id_in_store'])
        ...
        order, _ = Order.objects.update_or_create(
            id=order_data['order_id_in_store'],
...
def create_order_in_api(data):
    ...
    order_json = {
        ...
        'order_id_in_store': data['order_id_in_store']
    }
    ...
    message = f'''Here are your order details:\nItems:\n{', '.join(f"{Book.objects.get(id_in_warehouse=k).title}: {v}" for k, v in data['order_items'].items())}\nDelivery address: {data['delivery_address']}'''  # noqa
    from_email = settings.DEFAULT_FROM_EMAIL
